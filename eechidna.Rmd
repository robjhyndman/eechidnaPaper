...
```{r packages, echo=TRUE, cache=FALSE, message=FALSE}
...
```{r join_census_and_election, echo=FALSE}
...
census_and_election_subset <-  
  ...
  dplyr::select(Electorate, 
         ...
         Population:NotOwned)  %>% 
  dplyr::rename(ALP_perc = Average_Australian_Labor_Party_Percentage_in_electorate)
...
map.winners.grouped <- mutate(map.winners, 
    PartyNm = ifelse(PartyNm %in% coalition,
...
ggplot(data=map.winners.grouped, aes(map_id=id)) + 
  geom_map(aes(fill=PartyNm), map=nat_map_2016) +
...
ggplot(data=map.winners.grouped, aes(map_id=id)) + 
  ...
  geom_point(data=map.winners.grouped, aes(x=x, y=y, colour=PartyNm),
